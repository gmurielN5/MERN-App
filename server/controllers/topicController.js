...
exports.topic_list = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      topics,
      message: { msgBody: "topics succesfully fetched", msgError: false },
    })
  } catch (error) {
    return next(error)
  }
...
exports.topic_detail = async (req, res, next) => {
  try {
    ...
    const articlesByTopic = await Article.find({ topic: req.params.id })
    if (!topic) {
...
exports.topic_create = async (req, res, next) => {
  try {
    ...
    if (ReqTopic) {
      ...
      return res.status(201).json({
        topic,
        message: { msgBody: "topic succesfully created", msgError: false },
      })
    }
  } catch (error) {
    return next(error)
